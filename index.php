...
function showList($place){
	...
	$p=$place;
	$sql="select * from `" . $xoopsDB->prefix('forth_grade') . "` where `place`=$p order by id";
	...
	$tableT1=array_fill(0, 100, 1000);
  	$sql_getT1="select * from `" . $xoopsDB->prefix('forth_check1') . "` order by right_num1";
	...
	$tableT2=array_fill(0, 100, 1000);
  	$sql_getT2="select * from `" . $xoopsDB->prefix('forth_check2') . "` order by right_num2";
	...
	$tableT3=array_fill(0, 100, 1000);
  	$sql_getT3="select * from `" . $xoopsDB->prefix('forth_check3') . "` order by right_num3";
	...
	$tablePR4=array_fill(0, 300, 0);
  	$sql_getT4="select * from `" . $xoopsDB->prefix('forth_check4') . "` order by right_num4";
	...
	$placeList.="
				...
				";
	//先取得該試場所有學生的sn
	$sql_sn="select `sn` from `" . $xoopsDB->prefix('forth_grade') . "` where `place`=$p order by id";
...
function save($place,$test1,$test2,$test3,$absense){
	...
	//抓取該試場所有的sn
	$sql="select sn from " . $xoopsDB->prefix('forth_grade')  . " where place=$place order by id";
	//die($sql);
	$result=$xoopsDB->query($sql);
	while(list($sn)=$xoopsDB->fetchRow($result)){
		...
		$gpr=$tablePR4[$g4];
		$noCome=$absense[$sn];
		$sql_update="update " . $xoopsDB->prefix('forth_grade') . " set test1='$g1',t1='$gt1',test2='$g2',t2='$gt2',test3='$g3',t3='$gt3',tall='$g4',allt='$gt4',PR='$gpr',note='$noCome' where sn=$sn";
...
";
        ...
        $content.="<h3 align='center'>初試成績計算表（四年級）</h3>";
...
//	$content.="第" . $p . "試場成績<br>";
	...
	while(list($sn, $place, $id, $test1, $t1, $test2, $t2, $test3, $t3, $tall, $allt, $PR, $note)=$xoopsDB->fetchRow($result)){
		if ($note==1){
			$content.="<tr align=\"center\"><td>$i</td><td>$id</td><td>缺考</td><td>0</td><td>缺考</td><td>0</td><td>缺考</td><td>0</td><td>0</td><td>0</td><td>0</td></tr>";
		}else{
			$content.="<tr align=\"center\"><td>$i</td><td>$id</td><td>$test1</td><td>$t1</td><td>$test2</td><td>$t2</td><td>$test3</td><td>$t3</td><td>$tall</td><td>$allt</td><td>$PR</td></tr>";
		}
		$i++;
	}
	$content.="</table><br>";
	$content.="輸入人員：　　　　　　　　　　　　主試人員：　　　　　　　　　　　　" . date("Y-m-d h:i:sa");
	echo $content;
}
