...
function check3Show(){
  global $xoopsDB;
  $sql="select * from `" . $xoopsDB->prefix('forth_check3') . "` order by right_num3";
...
function importExcel(){
  ...
  if (preg_match('/\.(xlsx)$/i', $_FILES['excel']['name'])) {
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
      ...
      $sql = "insert into `" . $xoopsDB->prefix('forth_check3') . "` (`right_num3`, `T3`, `PR3`) values ('{$v[0]}' , '{$v[1]}' , '{$v[2]}')";
...
function delAll(){
  global $xoopsDB;
  $sql="delete from `" . $xoopsDB->prefix('forth_check3') . "`";
  $xoopsDB->queryF($sql) or die($sql);
}
