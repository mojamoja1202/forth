...
function importExcel(){
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
    	...
    	$v="";
  		...
  		//echo $val;
  		//echo "<br>";
  		//這邊要把資料放入兩張資料表：forth_student && forth_grade
  		//處理forth_student
  		$sql_student = "insert into `" . $xoopsDB->prefix('forth_student') . "` (`id`, `name`, `sex`, `year`, `month`, `day`, `school`, `class`, `phone`, `note`, `place`) values ('{$v[0]}' , '{$v[1]}' , '{$v[2]}' , '{$v[3]}' , '{$v[4]}' , '{$v[5]}' , '{$v[6]}' , '{$v[7]}' , '{$v[8]}' , '{$v[9]}' , '{$v[10]}')";
  		$xoopsDB->queryF($sql_student) or die($sql_student);

  		//處理forth_grade
  		$sql_grade = "insert into `" . $xoopsDB->prefix('forth_grade') . "` (`place`, `id`) values ('{$v[10]}' , '{$v[0]}')";
...
function delAll(){
  global $xoopsDB;
  $sql="delete from `" . $xoopsDB->prefix('forth_student') . "`";
  $xoopsDB->queryF($sql) or die($sql);
  $sqlDelgrade="delete from `" . $xoopsDB->prefix('forth_grade') . "`";
