...
function classShow($place){
  $p=$place;
  global $xoopsDB;
  $sql="select sn,id from `" . $xoopsDB->prefix('forth_grade') . "` where `place`=$p order by id";
...
function del($sn){
  global $xoopsDB;
  $sql="delete from `" . $xoopsDB->prefix('forth_grade') . "` where sn=$sn";
...
function updateForm($sn){
  global $xoopsDB;
  $sql="select sn,id from `" . $xoopsDB->prefix('forth_grade') . "` where `sn`=$sn";
...
function update($sn){
  global $xoopsDB;
  $sql="update `" . $xoopsDB->prefix('forth_grade') . "` set
