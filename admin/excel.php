...
//縮修的學生
$sql_short="select id,name from " . $xoopsDB->prefix('forth_student') . " where note='縮修'";
$result_short=$xoopsDB->query($sql_short);
$i=3;
while(list($id,$name)=$xoopsDB->fetchRow($result_short)){
	$sql_shortstudent="select * from " . $xoopsDB->prefix('forth_grade') . " where id='$id'";
...
//標題
$Y=date("Y")-1911;
$objActSheet->setCellValueByColumnAndRow(0, 1, "高雄市" . $Y . "學年度一般智能資賦優異學生鑑定初試成績統計表(四年級)");
$objActSheet->setCellValueByColumnAndRow(0, 2, "編號");
...
$sql="select * from " . $xoopsDB->prefix('forth_grade');
...
//標題
$Y=date("Y")-1911;
$objActSheet->setCellValueByColumnAndRow(0, 1, "高雄市" . $Y . "學年度一般智能資賦優異學生鑑定初試學生成績暨鑑定結果一覽表(四年級)");
$objActSheet->setCellValueByColumnAndRow(0, 2, "編號");
...
$objActSheet->setCellValueByColumnAndRow(4, 2, "T分數70分以上");
$objActSheet->setCellValueByColumnAndRow(5, 2, "T分數69分以上");
$objActSheet->setCellValueByColumnAndRow(6, 2, "T分數68分以上");
$objActSheet->setCellValueByColumnAndRow(7, 2, "T分數67分以上");
$objActSheet->setCellValueByColumnAndRow(8, 2, "T分數66分以上");
$objActSheet->setCellValueByColumnAndRow(9, 2, "T分數65分以上");
$objActSheet->setCellValueByColumnAndRow(10, 2, "T分數64分以上");
$objActSheet->setCellValueByColumnAndRow(11, 2, "T分數63分以上");
...
while($gg<34){
	...
	$t63=0;
	//抓取各間學校資料
	$sql_getschool="select allt,note from " . $xoopsDB->prefix('forth_grade') . " where id like '____" . $item[$gg] . "___'";
	...
	while(list($allt,$note)=$xoopsDB->fetchRow($result_getschool)){
		...
		$objActSheet->setCellValueByColumnAndRow(4, $k, $t70);
		$objActSheet->setCellValueByColumnAndRow(5, $k, $t69);
		$objActSheet->setCellValueByColumnAndRow(6, $k, $t68);
		$objActSheet->setCellValueByColumnAndRow(7, $k, $t67);
		$objActSheet->setCellValueByColumnAndRow(8, $k, $t66);
		$objActSheet->setCellValueByColumnAndRow(9, $k, $t65);
		$objActSheet->setCellValueByColumnAndRow(10, $k, $t64);
		$objActSheet->setCellValueByColumnAndRow(11, $k, $t63);
...
header('Content-Disposition: attachment;filename='.iconv('UTF-8','Big5','四年級總表').'.xls');
